 ...
 * 
 * @modified_by Stephen Ralph Moraga | Mark Pama
 */
class CI_Cart {
	...
	public function __construct($params = array())
	{
		...
		else
		{
			...
			$this->_cart_contents['sub_total'] = 0;
		}
	...
	function _save_cart()
	{
		...
		unset($this->_cart_contents['sub_total']);
		...
		foreach ($this->_cart_contents as $key => $val)
		{
			...
			if ( ! is_array($val) OR ! isset($val['price']) OR ! isset($val['qty']))
			{
				continue;
                        ...
                        if(isset($val['discount'])) {
                            $discount = ($val['discount'] / 100) * ($val['price'] * $val['qty']);
                            $sum = ($val['price'] * $val['qty']) - $discount;
                            $total += $sum;
                        } else {
			...
			// Set the subtotal
			// Product item subtotal (item * qty - discount)
                        if(isset($this->_cart_contents[$key]['discount'])) {
                            $discount = ($this->_cart_contents[$key]['discount'] / 100) * ($this->_cart_contents[$key]['price'] * $this->_cart_contents[$key]['qty']);
                            $sum = ($this->_cart_contents[$key]['price'] * $this->_cart_contents[$key]['qty']) - $discount;
                            $this->_cart_contents[$key]['subtotal'] = $sum;
                        } else {
                            $this->_cart_contents[$key]['subtotal'] = $sum = ($this->_cart_contents[$key]['price'] * $this->_cart_contents[$key]['qty']);
                        }
		...
		$this->_cart_contents['sub_total'] = $product_subtotal;
	 ...
	 * @return	integer
	 * 
	 * Call the cart_model->total instead - mark
	 * 
	 */
	function total()
	{
	...
	/**
	 * Sub Total Items(without shipping & misc)
	 *
	 * Returns the Sub Total count
	 *
	 * @access	public
	 * @return	integer
	 * @author - added by mark
	 */
	function subtotal()
	{
		return $this->_cart_contents['sub_total'];
	}
