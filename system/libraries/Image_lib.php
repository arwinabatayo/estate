...
class CI_Image_lib {
	...
	function initialize($props = array())
	{
		...
		/*
		 * Is get_assets/imagesize() Available?
		 *
		 ...
		 *
		 */
		if ( ! function_exists('get_assets/imagesize'))
		{
	...
	function image_process_gd($action = 'resize')
	{
		...
		if ($this->image_type == 3) // png we can actually preserve transparency
		{
			...
			_assets/imagesavealpha($dst_img, TRUE);
		}
		...
		if ($this->rotation_angle == 'hor')
		{
			for ($i = 0; $i < $height; $i++)
			{
				...
				while ($left < $right)
				{
					...
					_assets/imagesetpixel($src_img, $left, $i, $cr);
					_assets/imagesetpixel($src_img, $right, $i, $cl);
		...
		else
		{
			for ($i = 0; $i < $width; $i++)
			{
				...
				while ($top < $bot)
				{
					...
					_assets/imagesetpixel($src_img, $i, $top, $cb);
					_assets/imagesetpixel($src_img, $i, $bot, $ct);
	...
	function overlay_watermark()
	{
		...
		{
			// set our RGB value from above to be transparent and merge the _assets/images with the specified opacity
	...
	function text_watermark()
	{
		...
		{
			if ($this->wm_use_drop_shadow)
				_assets/imagestring($src_img, $this->wm_font_size, $x_shad, $y_shad, $this->wm_text, $drp_color);
				_assets/imagestring($src_img, $this->wm_font_size, $x_axis, $y_axis, $this->wm_text, $txt_color);
		}
	...
	function get_image_properties($path = '', $return = FALSE)
	{
		...
		$vals = @get_assets/imagesize($path);
	 ...
	 * source__assets/images with multiple periods, like:  my.cool.jpg
